...
ssize_t read_n(int fd, ssize_t n, void * buf) {
  ssize_t re = 0;
  while (re < n) {
    ssize_t r = read(fd, (char *)buf + re, n - re);
    if (r == -1) die("read");
    if (r == 0) break;
    re += r;
  }
...
ssize_t write_n(int fd, ssize_t n, void * buf) {
  ssize_t wr = 0;
  while (wr < n) {
    ssize_t w = write(fd, (char *)buf + wr, n - wr);
    if (w == -1) die("write");
    wr += w;
  }
  return wr;
...
void fourier_transform(sample_t *buf, long n, complex double *Y) {
  if (!pow2check(n)) {
    fprintf(stderr, "error : n (%ld) not a power of two\n", n);
    exit(1);
  }
  complex double * X = (complex double *)calloc(sizeof(complex double), n);
  while (1) {
    /* 複素数の配列に変換 */
